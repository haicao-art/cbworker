<?php
# @Author: crababy
# @Date:   2018-03-24T11:49:14+08:00
# @Last modified by:   crababy
# @Last modified time: 2018-03-24T11:50:15+08:00
...
class RedisDb
{
  ...
  public function __construct($config = null)
  {
    ...
    $ret = @$this->redis->connect($this->settings['host'], $this->settings['port']);
    if (!$ret) {
      throw new Exception('Redis Connection Refused', 1000);
    }
    if (false != $this->settings['auth']) {
      $this->redis->auth($this->settings['auth']);
    }
  ...
  {
    $args = func_get_args();
    MLogger::info('RedisCommands Args', $args);
    $ret = false;
    if (!isset($args[0])) {
      return $ret;
    }
    ...
    try {
      ...
      MLogger::info('RedisError', ['message' => $e->getMessage(), 'code' => $e->getCode()]);
      throw new Exception($e->getMessage(), $e->getCode());
    }
    MLogger::info('RedisCommands Result', ['result' => $ret]);
    return $ret;
  }
