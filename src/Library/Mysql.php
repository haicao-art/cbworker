...
class Mysql
{
  ...
  protected function connect()
  {
    try {
      ...
      $this->logger->error('Mysql Connect Error:' . $ex->getMessage());
      throw new Exception('数据库连接异常', (int)$ex->getCode());
    }
  }
  ...
  {
    $query = trim($query);
    $this->logger->info('Mysql Execute Query:' . json_encode([$query, $params]));
    ...
    $this->logger->info('Mysql Execute Result:' . json_encode($result));
    return $result;
  }
  ...
  {
    $this->logger->info('Mysql CommitTrans CommitTrans Commit');
  ...
  public function rollBackTrans()
  {
    if ($this->pdo->inTransaction()) {
      $this->logger->info('Mysql RollBackTrans RollBackTrans End');
